...
function App() {
  ...
  useEffect(() => {
    const ws = new WebSocket('ws://158.220.125.26:5001/transactions');
    ws.onopen = () => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        setTransactions(prev => {
          const updated = [tx, ...prev.filter(t => t.signature !== tx.signature)].slice(0, 50);
          return updated;
        });
      } catch (err) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            < TransactionFeed transactions={transactions} timeframe={timeframe} onTimeframeChange={handleTimeframeChange} onTransactionTypeChange={handleTransactionTypeChange} />
          </div>
        </div>
      </div>
    </div>
