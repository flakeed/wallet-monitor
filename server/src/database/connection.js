...
class Database {
  constructor() {
    this.pool = new Pool({
      ...
      max: 50, // Increased for bulk operations
    ...
    try {
      const query = `
        SELECT 
          ...
          COALESCE(SUM(sol_spent), 0) as total_spent_sol,
          COALESCE(SUM(sol_received), 0) as total_received_sol,
