...
const { Connection, PublicKey, getAssociatedTokenAddressSync } = require('@solana/web3.js');
...
app.get('/api/tokens/tracker', async (req, res) => {
  try {
    ...
    // Collect unique mints for batch price fetching
    const uniqueMints = [...new Set(rows.map(row => row.mint))];
    const prices = new Map();
    await Promise.all(uniqueMints.map(async (mint) => {
      const decimals = rows.find(r => r.mint === mint).decimals || 6;
      const price = await getTokenPrice(mint, decimals);
    ...
    // Calculate PnL and held for each wallet in tokens
    await Promise.all(result.map(async (t) => {
      await Promise.all(t.wallets.map(async (w) => {
        const held = await getCurrentBalance(connection, w.address, t.mint) || (w.tokensBought - w.tokensSold);
        w.held = held;

        let avgCost = 0;
        if (w.tokensBought > 0) {
          avgCost = w.solSpent / w.tokensBought;
        }

        const currentPrice = prices.get(t.mint) || 0;
        w.currentPrice = currentPrice;
        w.avgCost = avgCost;

        w.realizedPnL = +(w.solReceived - (avgCost * w.tokensSold)).toFixed(6);
        w.unrealizedPnL = +(held * (currentPrice - avgCost)).toFixed(6);
        w.pnlSol = +(w.realizedPnL + w.unrealizedPnL).toFixed(6); // Total PnL
      }));
    }));
