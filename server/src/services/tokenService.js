...
const REQUEST_DELAY = 50; // Reduced delay for faster processing
const TOKEN_CACHE_TTL = 24 * 60 * 60;
const PRICE_CACHE_TTL = 60 * 60;
const PROMISE_TTL = 60; // Reduced TTL for faster cleanup
let lastPriceRequest = 0;
const PRICE_REQUEST_DELAY = 100; // Reduced delay for price requests
...
async function processQueue() {
  ...
  while (true) {
    const requestData = await redis.lpop('helius:queue', 50); // Batch process up to 50 requests
