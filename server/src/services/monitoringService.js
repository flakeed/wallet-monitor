...
class WalletMonitoringService {
    ...
    async addWallet(address, name = null, groupId = null) {
        try {
            ...
            console.log(`[${new Date().toISOString()}] ✅ Added wallet: ${name || address.slice(0, 8)}...${groupId ? ` to group ${groupId}` : ''}`);
            return wallet;
        } catch (error) {
    ...
    async removeAllWallets(groupId = null) {
        try {
            ...
            const transactions = await this.db.getRecentTransactions(24 * 7, null, groupId);
