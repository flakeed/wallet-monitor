...
class WalletMonitoringService {
  ...
  async processTransaction(sig, wallet) {
    try {
      ...
      if (transactionType === 'buy') {
        pnl = -totalSolAmount * solPrice; // Cost in USD
      } else if (transactionType === 'sell') {
        const tokenValues = tokenChanges.reduce((sum, tc) => sum + (tc.usdValue || 0), 0);
        pnl = tokenValues - (totalSolAmount * solPrice); // Revenue - cost
      }
      return await this.db.withTransaction(async (client) => {
        ...
        const query = `
          INSERT INTO transactions (
            ...
            sol_spent, sol_received, usdc_spent, usdc_received, pnl
          )
  ...
  async analyzeTokenChanges(meta, transactionType, walletAddress) {
    ...
    const priceData = await this.priceService.getTokenPrices(mints);
    for (const [mint, aggregatedChange] of mintChanges) {
      ...
      const priceInfo = priceData.get(mint) || {
        price: 0,
        marketCap: 0,
        deploymentTime: null
      };
      tokenChanges.push({
        ...
        name: tokenInfo.name,
        usdValue: (aggregatedChange.totalRawChange / Math.pow(10, aggregatedChange.decimals)) * priceInfo.price,
        marketCap: priceInfo.marketCap,
        deploymentTime: priceInfo.deploymentTime
      });
      console.log(`[${new Date().toISOString()}] ✅ Added token change: ${tokenInfo.symbol} (${aggregatedChange.totalRawChange} total raw units, USD: ${priceInfo.price})`);
    }
  ...
  async saveTokenOperationInTransaction(client, transactionId, tokenChange, transactionType) {
    try {
      ...
      const operationQuery = `
        INSERT INTO token_operations (transaction_id, token_id, amount, operation_type, usd_value, market_cap, deployment_time)
      ...
      await client.query(operationQuery, [
        ...
        amount,
        transactionType,
        tokenChange.usdValue,
        tokenChange.marketCap,
        tokenChange.deploymentTime
      ]);
    } catch (error) {
